...
G4cout << TrackID << " " << PosX << " " << PosY << " " << PosZ << " " << KineticEnergy << " " << MomentumMagnitude << " " << Volume->GetName() << " " << ProcessName <<  G4endl;
std::ofstream TrackMuonFile;
TrackMuonFile.open("waste_barrel_U.dat", std::ios::app);
TrackMuonFile << TrackID << " " << PosX << " " << PosY << " " << PosZ << " " << KineticEnergy << " " << MomentumMagnitude << " " << Volume->GetName() << " " << ProcessName <<  G4endl;
TrackMuonFile.close();
}
}
}
if (Volume->GetName() == "TargetMaterial1" || Volume->GetName() == "TargetMaterial2" || Volume->GetName() == "TargetMaterial3" ) {
if (ParticleName == "mu-") {
if (ProcessName == "muMinusCaptureAtRest"){
std::ofstream AbsorptionFile;
AbsorptionFile.open("Absorption_waste_barrel_U.dat", std::ios::app);
AbsorptionFile << TrackID << " " << PosX << " " << PosY << " " << PosZ << " " << KineticEnergy << " " << MomentumMagnitude << " " << Volume->GetName() << " " << ProcessName <<  G4endl;
AbsorptionFile.close();
}
}
}
}
